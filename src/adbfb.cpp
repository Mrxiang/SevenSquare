...
//xshx add 2020.4.11
int ADBDevice::probeDeviceInformation(void)
{
    AdbExecutor adb;
    QString     sInformation;
    adb.addArg("shell");
    adb.addArg(SYS_CRITICAL_TEMP);
    adb.run();
    sInformation = adb.output.simplified();
    DT_TRACE(" critical temp: " << sInformation);

    adb.clear();
    adb.addArg("shell");
    adb.addArg(SYS_PASSIVE_TEMP );
    adb.run();
    sInformation = adb.output.simplified();
    DT_TRACE(" critical temp: " << sInformation);
...
//xshx add
int ADBFrameBuffer::printTemp(QByteArray &bytes, int offset)
{
    AdbExecutor adb;
    QStringList args;
    args.clear();
    ...
    adb.run(args);
    if ( adb.exitSuccess()) {
        DT_TRACE("TMEP ->" << adb.output.simplified() );
    }else{
        adb.printErrorInfo();
    }
    //end add
